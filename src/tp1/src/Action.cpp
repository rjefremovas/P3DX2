...
void Action::avoidObstacles(std::vector<float> speed)
{
    ...
    float frontDist = lasers[front];
    float rightDist = lasers[right];
    


    if (frontDist < ideal_distance) {
        linVel = 0.0;
        angVel = 0.1;
    }
...
void Action::keepAsFarthestAsPossibleFromWalls(std::vector<float> lasers, std::vector<float> sonars)
{
    ...
    std::cout << "blabla" << sonars[0] << lasers[0] << "\n";
